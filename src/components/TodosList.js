...
const TodosList = ({
  ...
  function handleActive(active) {
    setActive(active.currentTarget.value);
    handleFilter(active.currentTarget.value);
  }
  ...
  return (
    <Box classname={isDarkMode ? "dark" : ""} sx={{ width: "100%" }}>
      <Box>
        {todos.map((todo) => (
      ...
      >
        <Box component="span" sx={{ fontSize: ".9rem", fontWeight: "900" }}>
          {/* {todos.filter((a) => a.complete === false).length} items left */}
